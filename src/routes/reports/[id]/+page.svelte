...
<main>
    ...
    <span class="city"> {report.city} </span> <br />

    <!--TODO: Sanitize input-->
    <p> {@html formatInfo(report.info)} </p>
    
    {#if report.accountable}
    <p>
        <span>Accountable person:</span>
        <a href="/profiles/{report.accountable.id}" class="icon" style="color: var(--primary)">
            <span>
                {report.accountable.name}
            </span>
            <span>
